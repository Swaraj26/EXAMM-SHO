...
IslandSpeciationStrategy::IslandSpeciationStrategy(
    ...
    // Todo AK Remove the commented code
...
// Todo AK Remove comments
// void IslandSpeciationStrategy::generate_initial_hyperparameters(double &learning_rate)
// {
//     //TODO convert these variables to class variables
...
//     // learning_rate_delta = (rng_0_1(generator) * (initial_learning_rate_delta_max - initial_learning_rate_delta_min)) + initial_learning_rate_delta_min;
//     // dropout_probability = (rng_0_1(generator) * (initial_dropout_probability_max - initial_dropout_probability_min)) + initial_dropout_probability_min;
// }

// Todo AK Remove comments
// void EXAMM::generate_simplex_hyperparameters(double &learning_rate, double &learning_rate_delta, double &dropout_probability)
// void IslandSpeciationStrategy::generate_simplex_hyperparameters(double &learning_rate)
// {
...
//         // Log::error("\n SY: Island size 1: %d",islands.size());
//         int x_rnd = rng_0_1(generator) * islands.size();
//         // Log::error("\n SY: x_rnd = %d",x_rnd);
//         // Log::error("\n SY: Island size 2: %d",islands[x_rnd]->size());

//         if(islands[x_rnd]->size() > 0){
//             int y_rnd = rng_0_1(generator) * islands[x_rnd]->size();
//         // Log::error("\n SY: y_rnd = %d", y_rnd);
//         // Log::error("\n SY: During current genome");

        
//         RNN_Genome *current_genome = islands[x_rnd]->get_genome_at(y_rnd);
...
RNN_Genome* IslandSpeciationStrategy::generate_genome(uniform_real_distribution<double> &rng_0_1, minstd_rand0 &generator, function<void (int32_t, RNN_Genome*)> &mutate, function<RNN_Genome* (RNN_Genome*, RNN_Genome *)> &crossover, int32_t number_stir_mutations=0) {
    ...
    if (island->is_initializing()) {
        ...
        // Log::error("\n SY: learning rate after updating genome: %lf",genome->get_learning_rate());
