...
RNN_Genome::RNN_Genome(vector<RNN_Node_Interface*> &_nodes,
    ...
    rng_lr = uniform_real_distribution<double>(0.0001, 0.03);
    rng_ilr = uniform_real_distribution<double>(0.001, 0.05);

    rng_cr = uniform_real_distribution<double>(0.5, 1.0);
    rng_icr = uniform_real_distribution<double>(0.5, 1.0);

    rng_aer = uniform_real_distribution<double>(0.5, 1.0);
    rng_iaer = uniform_real_distribution<double>(0.5, 1.0);

    rng_arer = uniform_real_distribution<double>(0.5, 1.0);
    rng_iarer = uniform_real_distribution<double>(0.5, 1.0);

    rng_eer = uniform_real_distribution<double>(0.5, 1.0);
    rng_ieer = uniform_real_distribution<double>(0.5, 1.0);

    rng_der = uniform_real_distribution<double>(0.5, 1.0);
    rng_ider = uniform_real_distribution<double>(0.5, 1.0);

    rng_ser = uniform_real_distribution<double>(0.0, 0.00001);
    rng_iser = uniform_real_distribution<double>(0.0, 0.00001);

    rng_anr = uniform_real_distribution<double>(0.5, 1.0);
    rng_ianr = uniform_real_distribution<double>(0.5, 1.0);

    rng_enr = uniform_real_distribution<double>(0.5, 1.0);
    rng_ienr = uniform_real_distribution<double>(0.5, 1.0);

    rng_dnr = uniform_real_distribution<double>(0.5, 1.0);
    rng_idnr = uniform_real_distribution<double>(0.5, 1.0);

    rng_snr = uniform_real_distribution<double>(0.5, 1.0);
    rng_isnr = uniform_real_distribution<double>(0.5, 1.0);

    rng_mnr = uniform_real_distribution<double>(0.5, 1.0);
    rng_imnr = uniform_real_distribution<double>(0.5, 1.0);
    ...
    clone_rate_min = 0.5;
    clone_rate_max = 1.0;
    clone_rate = rng_cr(generator);
    initial_clone_rate = rng_icr(generator);

    initial_add_edge_rate_min = 0.5;
    initial_add_edge_rate_max = 1.0;
    add_edge_rate_min = 0.5;
    add_edge_rate_max = 1.0;
    add_edge_rate = rng_aer(generator);
    initial_add_edge_rate = rng_iaer(generator);
    ...
    add_recurrent_edge_rate = rng_arer(generator);
    initial_add_recurrent_edge_rate = rng_iarer(generator);

    initial_enable_edge_rate_min = 0.5;
    initial_enable_edge_rate_max = 1.0;
    enable_edge_rate_min = 0.5;
    enable_edge_rate_max = 1.0;
    enable_edge_rate = rng_eer(generator);
    initial_enable_edge_rate = rng_ieer(generator);

    initial_disable_edge_rate_min = 0.5;
    initial_disable_edge_rate_max = 1.0;
    disable_edge_rate_min = 0.5;
    disable_edge_rate_max = 1.0;
    disable_edge_rate = rng_der(generator);
    initial_disable_edge_rate = rng_ider(generator);

    initial_split_edge_rate_min = 0.0;
    initial_split_edge_rate_max = 0.0;
    split_edge_rate_min = 0.0;
    split_edge_rate_max = 0.0;
    split_edge_rate = 0.0;
    initial_split_edge_rate = 0.0;

    initial_add_node_rate_min = 0.5;
    initial_add_node_rate_max = 1.0;
    add_node_rate_min = 0.5;
    add_node_rate_max = 1.0;
    add_node_rate = rng_anr(generator);
    initial_add_node_rate = rng_ianr(generator);

    initial_enable_node_rate_min = 0.5;
    initial_enable_node_rate_max = 1.0;
    enable_node_rate_min = 0.5;
    enable_node_rate_max = 1.0;
    enable_node_rate = rng_enr(generator);
    initial_enable_node_rate = rng_ienr(generator);

    initial_disable_node_rate_min = 0.5;
    initial_disable_node_rate_max = 1.0;
    disable_node_rate_min = 0.5;
    disable_node_rate_max = 1.0;
    disable_node_rate = rng_dnr(generator);
    initial_disable_node_rate = rng_idnr(generator);

    initial_split_node_rate_min = 0.5;
    initial_split_node_rate_max = 1.0;
    split_node_rate_min = 0.5;
    split_node_rate_max = 1.0;
    split_node_rate = rng_snr(generator);
    initial_split_node_rate = rng_isnr(generator);

    initial_merge_node_rate_min = 0.5;
    initial_merge_node_rate_max = 1.0;
    merge_node_rate_min = 0.5;
    merge_node_rate_max = 1.0;
    merge_node_rate = rng_mnr(generator);
    initial_merge_node_rate = rng_imnr(generator);




    learning_rate = rng_lr(generator);
    initial_learning_rate = rng_ilr(generator);
    // Log::error("SY: Constructor learning rate %lf",learning_rate);
    // Log::error("SY: Constructor initial learning rate %lf",initial_learning_rate);
...
double RNN_Genome::get_learning_rate() const{
    return learning_rate;
}



double RNN_Genome::get_clone_rate() const{
    return clone_rate;
}

double RNN_Genome::get_initial_clone_rate() const{
    return initial_clone_rate;
}

double RNN_Genome::get_best_clone_rate() const{
    return best_clone_rate;
}

void RNN_Genome::set_clone_rate(double _clone_rate) {
    clone_rate = _clone_rate;
}


double RNN_Genome::get_add_edge_rate() const{
    return add_edge_rate;
}

double RNN_Genome::get_initial_add_edge_rate() const{
    return initial_add_edge_rate;
}

double RNN_Genome::get_best_add_edge_rate() const{
    return best_add_edge_rate;
}

void RNN_Genome::set_add_edge_rate(double _add_edge_rate) {
    add_edge_rate = _add_edge_rate;
}



double RNN_Genome::get_add_recurrent_edge_rate() const{
    return add_recurrent_edge_rate;
}

double RNN_Genome::get_initial_add_recurrent_edge_rate() const{
    return initial_add_recurrent_edge_rate;
}

double RNN_Genome::get_best_add_recurrent_edge_rate() const{
    return best_add_recurrent_edge_rate;
}

void RNN_Genome::set_add_recurrent_edge_rate(double _add_recurrent_edge_rate) {
    add_recurrent_edge_rate = _add_recurrent_edge_rate;
}


double RNN_Genome::get_enable_edge_rate() const{
    return enable_edge_rate;
}

double RNN_Genome::get_initial_enable_edge_rate() const{
    return initial_enable_edge_rate;
}

double RNN_Genome::get_best_enable_edge_rate() const{
    return best_enable_edge_rate;
}

void RNN_Genome::set_enable_edge_rate(double _enable_edge_rate) {
    enable_edge_rate = _enable_edge_rate;
}



double RNN_Genome::get_disable_edge_rate() const{
    return disable_edge_rate;
}

double RNN_Genome::get_initial_disable_edge_rate() const{
    return initial_disable_edge_rate;
}

double RNN_Genome::get_best_disable_edge_rate() const{
    return best_disable_edge_rate;
}

void RNN_Genome::set_disable_edge_rate(double _disable_edge_rate) {
    disable_edge_rate = _disable_edge_rate;
}


double RNN_Genome::get_split_edge_rate() const{
    return split_edge_rate;
}

double RNN_Genome::get_initial_split_edge_rate() const{
    return initial_split_edge_rate;
}

double RNN_Genome::get_best_split_edge_rate() const{
    return best_split_edge_rate;
}

void RNN_Genome::set_split_edge_rate(double _split_edge_rate) {
    split_edge_rate = _split_edge_rate;
}


double RNN_Genome::get_add_node_rate() const{
    return add_node_rate;
}

double RNN_Genome::get_initial_add_node_rate() const{
    return initial_add_node_rate;
}

double RNN_Genome::get_best_add_node_rate() const{
    return best_add_node_rate;
}

void RNN_Genome::set_add_node_rate(double _add_node_rate) {
    add_node_rate = _add_node_rate;
}


double RNN_Genome::get_enable_node_rate() const{
    return enable_node_rate;
}

double RNN_Genome::get_initial_enable_node_rate() const{
    return initial_enable_node_rate;
}

double RNN_Genome::get_best_enable_node_rate() const{
    return best_enable_node_rate;
}

void RNN_Genome::set_enable_node_rate(double _enable_node_rate) {
    enable_node_rate = _enable_node_rate;
}



double RNN_Genome::get_disable_node_rate() const{
    return disable_node_rate;
}

double RNN_Genome::get_initial_disable_node_rate() const{
    return initial_disable_node_rate;
}

double RNN_Genome::get_best_disable_node_rate() const{
    return best_disable_node_rate;
}

void RNN_Genome::set_disable_node_rate(double _disable_node_rate) {
    disable_node_rate = _disable_node_rate;
}


double RNN_Genome::get_split_node_rate() const{
    return split_node_rate;
}

double RNN_Genome::get_initial_split_node_rate() const{
    return initial_split_node_rate;
}

double RNN_Genome::get_best_split_node_rate() const{
    return best_split_node_rate;
}

void RNN_Genome::set_split_node_rate(double _split_node_rate) {
    split_node_rate = _split_node_rate;
}

double RNN_Genome::get_merge_node_rate() const{
    return merge_node_rate;
}

double RNN_Genome::get_initial_merge_node_rate() const{
    return initial_merge_node_rate;
}

double RNN_Genome::get_best_merge_node_rate() const{
    return best_merge_node_rate;
}

void RNN_Genome::set_merge_node_rate(double _merge_node_rate) {
    merge_node_rate = _merge_node_rate;
}

// double RNN_Genome::get_learning_rate_delta() const {
//     return learning_rate_delta;
// }

// double RNN_Genome::get_dropout_probability() const {
//     return dropout_probability;
// }

double RNN_Genome::get_initial_learning_rate() const{
    return initial_learning_rate;
}

// double RNN_Genome::get_initial_learning_rate_delta() const {
//     return initial_learning_rate_delta;
// }

// double RNN_Genome::get_initial_dropout_probability() const {
//     return initial_dropout_probability;
// }

double RNN_Genome::get_best_learning_rate() const {
    return best_learning_rate;
}

// double RNN_Genome::get_best_learning_rate_delta() const {
//     return best_learning_rate_delta;
// }

// double RNN_Genome::get_best_dropout_probability() const {
//     return best_dropout_probability;
// }
...
void RNN_Genome::backpropagate(const vector< vector< vector<double> > > &inputs, const vector< vector< vector<double> > > &outputs, const vector< vector< vector<double> > > &validation_inputs, const vector< vector< vector<double> > > &validation_outputs) {
    ...
    Log::error("SY: learning rate backprop before: %lf",this->learning_rate);
...
void RNN_Genome::backpropagate_stochastic(const vector< vector< vector<double> > > &inputs, const vector< vector< vector<double> > > &outputs, const vector< vector< vector<double> > > &validation_inputs, const vector< vector< vector<double> > > &validation_outputs, bool random_sequence_length, int sequence_length_lower_bound, int sequence_length_upper_bound) {
    ...
    Log::error("SY: learning rate without arrow stochastic backprop before: %lf",learning_rate);
    int n_parameters = this->get_number_weights();
